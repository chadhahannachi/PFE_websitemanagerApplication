...
const LicenceDetail = () => {
    ...
    useEffect(() => {
        const fetchLicenceAndCompany = async () => {
            

            if (!id) {
                setError("ID de licence manquant");
                setLoading(false);
                return;
            }
            ...
            try {
                ...
                // Récupérer la licence par ID
                const licenceResponse = await axios.get(`http://localhost:5000/licences/${id}`);
                const licenceData = licenceResponse.data;
                setLicence(licenceData);

                // Récupérer l'entreprise via mongocompanyid de la licence
                if (licenceData.mongo_company_id) {
                    const entrepriseResponse = await axios.get(`http://localhost:5000/entreprises/${licenceData.mongo_company_id}`);
