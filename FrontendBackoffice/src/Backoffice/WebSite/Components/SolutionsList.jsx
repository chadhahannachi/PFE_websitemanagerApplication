...
const SolutionsList = () => {
  ...
  useEffect(() => {
    const fetchSolutionsByEntreprise = async () => {
      if (!token || !userId || !userEntreprise) { setLoading(false); return; }
      try {
        const config = { headers: { Authorization: `Bearer ${token}` } };
        const response = await axios.get(`http://localhost:5000/contenus/Solution/entreprise/${userEntreprise}`, config);
        setSolutions(response.data);
      } catch (error) {
        setSnackbar({ open: true, message: "Erreur lors de la récupération des solutions.", severity: "error" });
      } finally {
        setLoading(false);
      }
    };
    if (userEntreprise) fetchSolutionsByEntreprise();
  }, [userEntreprise]);
  ...
  const uploadImage = async () => {
    ...
    try {
      ...
      setCurrentSolution(prev => ({ ...prev, image: response.data.secure_url }));
  ...
  const handleSave = async () => {
    try {
      ...
      if (currentSolution._id) {
        await axios.patch(`http://localhost:5000/contenus/Solution/${currentSolution._id}`, currentSolution, config);
        setSnackbar({ open: true, message: "Solution modifiée avec succès !", severity: "success" });
      } else {
        if (!userEntreprise) throw new Error("L'entreprise de la solution n'est pas définie.");
        const payload = { ...currentSolution, datePublication: currentSolution.datePublication || new Date().toISOString(), entreprise: userEntreprise };
        await axios.post("http://localhost:5000/contenus/Solution", payload, config);
        setSnackbar({ open: true, message: "Solution créée avec succès !", severity: "success" });
      }
      setAddModalOpen(false);
      setEditModalOpen(false);
      setCurrentSolution({ _id: null, titre: '', description: '', image: '', isPublished: false, datePublication: '', entreprise: userEntreprise || '' });
      fetchSolutions();
    } catch (error) {
      setSnackbar({ open: true, message: error.message || "Erreur lors de la sauvegarde de la solution.", severity: "error" });
    }
  };
  ...
  const handleAdd = () => {
    setCurrentSolution({ titre: '', description: '', image: '', isPublished: false, datePublication: '', entreprise: userEntreprise || '' });
  ...
  const handleArchive = async () => {
    if (!archiveId) return;
    try {
      await axios.patch(`http://localhost:5000/contenus/Solution/${archiveId}/archive`, {}, { headers: { Authorization: `Bearer ${token}` } });
      setSolutions(prev => prev.filter(e => e._id !== archiveId));
      setSnackbar({ open: true, message: "Solution archivée avec succès !", severity: "success" });
    } catch (error) {
      setSnackbar({ open: true, message: "Erreur lors de l'archivage de la solution.", severity: "error" });
    } finally {
  ...
  const filteredSolutions = solutions.filter(solution =>
    solution.titre.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="card">
      <div className="card-header d-flex flex-wrap align-items-center justify-content-between gap-3">
        ...
        <div className="d-flex flex-wrap align-items-center gap-3">
            ...
            onClick={handleAdd}
          >
            <i className="ri-add-line" /> Add Solution
            </button>
          </div>
      </div>
      <div className="card-body">
            ...
            {filteredSolutions.map((solution, index) => (
              <tr key={solution._id}>
                <td>
                  ...
                  <td>{truncateText(solution.titre)}</td>
                  <td>{truncateText(solution.description)}</td>
                <td>
                  <img
                    src={solution.image || "https://via.placeholder.com/50"}
                    alt={solution.titre}
                ...
                <td>
                    {/* <Link
                    to={`/SolutionDetail/${solution._id}`}
                  ...
                  >
                    ...
                    <button
                      type="button"
                      onClick={() => handleAskArchive(solution._id)}
        ...
        <div className="d-flex flex-wrap align-items-center justify-content-between gap-2 mt-24">
          <span>Showing 1 to {filteredSolutions.length} of {filteredSolutions.length} entries</span>
            ...
            <div className="modal-dialog modal-dialog-centered modal-lg">
              <div className="modal-content p-4" style={{ border: 'none', boxShadow: '0 2px 16px rgba(0,0,0,0.12)' }}>
                <div className="modal-header" style={{ border: 'none', boxShadow: 'none' }}>
                  <h6 className="modal-title fw-semibold w-100 text-center" style={{ letterSpacing: '0.5px' }}>
                    {addModalOpen ? 'Ajouter une Solution' : 'Modifier la Solution'}
                  </h6>
              ...
              <input
                type="text"
                      ...
                      onChange={e => setCurrentSolution(prev => ({ ...prev, titre: e.target.value }))}
              ...
              />
                  ...
                  <div className="mb-3">
                    ...
                    <textarea
                      ...
                      onChange={e => setCurrentSolution(prev => ({ ...prev, description: e.target.value }))}
              ...
              />
                  ...
                  <div className="mb-3">
                    ...
                    {currentSolution.image && (
                      <img src={currentSolution.image} alt="Aperçu" className="mt-2" style={{ width: 'auto', maxWidth: '50%', maxHeight: '50%', objectFit: 'cover', borderRadius: '6px' }} onError={e => { e.target.src = 'https://via.placeholder.com/150'; }} />
              )}
            </div>
                </div>
          ...
          <div className="modal fade show" style={{display:'block',zIndex:1100}} tabIndex="-1">
            <div className="modal-dialog modal-dialog-centered">
              <div className="modal-content" style={{ border: 'none', boxShadow: '0 2px 16px rgba(0,0,0,0.12)' }}>
                <div className="modal-header" style={{ border: 'none', boxShadow: 'none' }}>
                  <h6 className="modal-title fw-semibold">Archivage de la solution</h6>
                ...
                <div className="modal-body" style={{ border: 'none', boxShadow: 'none' }}>
                  Êtes-vous sûr de vouloir archiver cette solution ?
                </div>
                <div className="modal-footer" style={{ border: 'none', boxShadow: 'none' }}>
