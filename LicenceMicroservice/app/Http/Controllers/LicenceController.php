...
class LicenceController extends Controller
{
    ...
    {
        return $licence->load('licenceRequest', 'payements');
    }
    
    public function getById($id)
    {
        $licence = Licence::with('licenceRequest', 'payements')->find($id);
        
        if (!$licence) {
            return response()->json(['message' => 'Licence not found'], 404);
        }
    ...
    public function store(Request $request)
    {
        ...
        if ($oldStatus !== 'validated') {
            $licenceRequest->notify(new \App\Notifications\LicenceRequestStatusUpdated($licenceRequest, $licence->id));
        }
